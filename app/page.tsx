...
export default function Home(){
 ...
 return <main className="min-h-screen">
  ...
  </div><div className={`quote-card ${quoteOpen?'open':''}`}>{!quoteOpen?<button className="quote-trigger" onClick={()=>setQuoteOpen(true)}><span><b>Generate customer quotation</b><small>Create a personalised PDF from this estimate</small></span><strong>Get PDF →</strong></button>:<div className="quote-form"><div className="quote-title"><div><b>Customer quotation</b><span>Enter a name and at least one contact method</span></div><button aria-label="Close quotation form" onClick={()=>{setQuoteOpen(false);setQuoteError('')}}>×</button></div><label>Customer name</label><input maxLength={60} value={customerName} onChange={e=>setCustomerName(e.target.value)} placeholder="Full name"/><div className="quote-fields"><div><label>Email address</label><input maxLength={80} type="email" value={customerEmail} onChange={e=>setCustomerEmail(e.target.value)} placeholder="name@example.com"/></div><div><label>Phone number</label><input maxLength={20} inputMode="tel" value={customerPhone} onChange={e=>setCustomerPhone(e.target.value)} placeholder="+91 98765 43210"/></div></div>{quoteError&&<p className="quote-error" role="alert">{quoteError}</p>}<button className="download-quote" onClick={generateQuotation}>Download PDF quotation</button><p className="privacy-note"><Icon name="shield" size={14}/> Generated on this device. Contact details are not stored.</p></div>}</div></div></section>
