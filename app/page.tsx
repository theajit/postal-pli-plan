...
export default function Home(){
 ...
 const [quoteOpen,setQuoteOpen]=useState(false),[customerName,setCustomerName]=useState(''),[customerEmail,setCustomerEmail]=useState(''),[customerPhone,setCustomerPhone]=useState(''),[quoteError,setQuoteError]=useState('');
 ...
 useEffect(()=>{if(!quoteOpen)return;const close=(event:KeyboardEvent)=>{if(event.key==='Escape'){setQuoteOpen(false);setQuoteError('');}};document.addEventListener('keydown',close);const previous=document.body.style.overflow;document.body.style.overflow='hidden';return()=>{document.removeEventListener('keydown',close);document.body.style.overflow=previous;};},[quoteOpen]);
 ...
 const generateQuotation=async()=>{
  ...
  if(name.length<2){setQuoteError('Please enter the customer name.');return;}
  if(!email&&!phone){setQuoteError('Please provide an email address or phone number.');return;}
  if(email&&!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)){setQuoteError('Please enter a valid email address.');return;}
  if(phone&&!/^[+\d][\d\s-]{7,15}$/.test(phone)){setQuoteError('Please enter a valid phone number.');return;}
  setQuoteError('');
  const {jsPDF}=await import('jspdf'); const doc=new jsPDF({unit:'mm',format:'a4'}); const nf=(v:number)=>new Intl.NumberFormat('en-IN',{maximumFractionDigits:0}).format(v); const today=new Date(); const quoteNo=`PP-${today.toISOString().slice(0,10).replaceAll('-','')}-${String(today.getTime()).slice(-6)}`;
  doc.setFillColor(190,25,39);doc.rect(0,0,210,38,'F');doc.setTextColor(255,255,255);doc.setFont('helvetica','bold');doc.setFontSize(22);doc.text('PostalPlan',18,17);doc.setFontSize(10);doc.setFont('helvetica','normal');doc.text('PLI & RPLI Planning Quotation',18,25);doc.text(`Quotation: ${quoteNo}`,192,16,{align:'right'});doc.text(today.toLocaleDateString('en-IN',{day:'2-digit',month:'short',year:'numeric'}),192,24,{align:'right'});
  doc.setTextColor(45,36,34);doc.setFont('helvetica','bold');doc.setFontSize(12);doc.text('Prepared for',18,52);doc.setFontSize(16);doc.text(name.slice(0,60),18,61);doc.setFont('helvetica','normal');doc.setFontSize(9);doc.setTextColor(105,91,86);doc.text([email&&`Email: ${email.slice(0,80)}`,phone&&`Phone: ${phone.slice(0,20)}`].filter(Boolean) as string[],18,68);
  doc.setDrawColor(230,219,208);doc.line(18,82,192,82);doc.setTextColor(45,36,34);doc.setFont('helvetica','bold');doc.setFontSize(14);doc.text(`${policy.group} ${policy.name} (${policy.hindi})`,18,94);doc.setFont('helvetica','normal');doc.setFontSize(9);doc.setTextColor(110,95,90);doc.text(r.official?'Premium basis: Matching supplied annexure table row':'Premium basis: Indicative planning estimate',18,101);
  const rows=[['Entry age',`${age} years`],['Maturity / ceasing age',`${policy.fixedTerm?age+term:maturityAge} years`],['Policy term',`${term} years`],['Sum assured',`Rs. ${nf(sumAssured)}`],['Payment frequency',frequency],['Total instalments',String(r.payments)],['First-year instalment',`Rs. ${nf(r.firstYear)}`],['Renewal instalment',`Rs. ${nf(r.renewal)}`],['Estimated total premium',`Rs. ${nf(r.totalPremium)}`],['Projected bonus',`Rs. ${nf(r.bonus)}`]];
  let y=114;rows.forEach(([label,value],i)=>{if(i%2===0){doc.setFillColor(251,247,239);doc.rect(18,y-6,174,10,'F');}doc.setTextColor(105,91,86);doc.setFont('helvetica','normal');doc.text(label,22,y);doc.setTextColor(45,36,34);doc.setFont('helvetica','bold');doc.text(value,188,y,{align:'right'});y+=10;});
  doc.setFillColor(255,239,207);doc.roundedRect(18,218,174,30,3,3,'F');doc.setTextColor(105,72,22);doc.setFontSize(10);doc.setFont('helvetica','normal');doc.text('PROJECTED MATURITY VALUE',25,229);doc.setFontSize(21);doc.setFont('helvetica','bold');doc.text(`Rs. ${nf(r.maturity)}`,25,241);doc.setFontSize(9);doc.text(`Estimated net gain: Rs. ${nf(r.gain)}`,185,240,{align:'right'});
  doc.setFont('helvetica','normal');doc.setFontSize(8);doc.setTextColor(115,104,100);const note='Important: This is an indicative planning quotation generated by PostalPlan, an independent calculator. It is not an official India Post quotation, policy offer or guarantee. Premiums, rebates, GST, bonuses, eligibility and benefits must be confirmed with India Post.';doc.text(doc.splitTextToSize(note,174),18,261);doc.setDrawColor(230,219,208);doc.line(18,280,192,280);doc.text('Generated privately in your browser - customer details are not stored by PostalPlan.',105,287,{align:'center'});
  doc.save(`PostalPlan-Quotation-${name.replace(/[^a-z0-9]+/gi,'-').replace(/^-|-$/g,'').slice(0,35)||'Customer'}.pdf`);
 };
 return <main className="min-h-screen">
  ...
  <section className="workspace" id="calculator"><div className="form-card">
  ...
  </div><div className="results-column"><div className="results-card">
   ...
   <div className="quote-card"><button className="quote-trigger" onClick={()=>setQuoteOpen(true)}><span><b>Generate customer quotation</b><small>Create a personalised PDF from this estimate</small></span><strong>Get PDF →</strong></button></div></div>{quoteOpen&&<div className="quote-backdrop" role="presentation" onMouseDown={()=>{setQuoteOpen(false);setQuoteError('')}}><div className="quote-modal" role="dialog" aria-modal="true" aria-labelledby="quote-heading" onMouseDown={e=>e.stopPropagation()}><div className="quote-modal-accent"/><div className="quote-form"><div className="quote-title"><div><span className="quote-kicker">PERSONALISED ESTIMATE</span><b id="quote-heading">Create customer quotation</b><span>We’ll use the calculation currently shown on screen.</span></div><button aria-label="Close quotation form" onClick={()=>{setQuoteOpen(false);setQuoteError('')}}>×</button></div><div className="quote-summary"><div><span>Policy</span><strong>{policy.hindi}</strong></div><div><span>Maturity estimate</span><strong>{money(r.maturity)}</strong></div></div><label>Customer name</label><input autoFocus maxLength={60} value={customerName} onChange={e=>setCustomerName(e.target.value)} placeholder="Full name"/><div className="quote-fields"><div><label>Email address</label><input maxLength={80} type="email" value={customerEmail} onChange={e=>setCustomerEmail(e.target.value)} placeholder="name@example.com"/></div><div><label>Phone number</label><input maxLength={20} inputMode="tel" value={customerPhone} onChange={e=>setCustomerPhone(e.target.value)} placeholder="+91 98765 43210"/></div></div><p className="contact-hint">Provide at least one contact method.</p>{quoteError&&<p className="quote-error" role="alert">{quoteError}</p>}<button className="download-quote" onClick={generateQuotation}>Download PDF quotation</button><p className="privacy-note"><Icon name="shield" size={14}/> Generated on this device. Contact details are not stored.</p></div></div></div>}</div></section>
